...
It refreshes from the database every minute and carries its own
...
| backoff 1m → 2m → 4m … 30m on failure, reset on success | stops hammering an edge that is already refusing you |
...
Chrome resolves `localhost` to **::1** first. uvicorn binds `0.0.0.0` — IPv4
only, because Python binds v6-only on Windows, so `--host ::` would trade LAN
access for loopback. A POST to `http://localhost:8790` therefore lands on an
address nothing is listening on, and is refused after a ~1.2s stall that the
pusher's catch can only report as a generic network error.

Measured on this machine:

| target | result |
| --- | --- |
| `http://[::1]:8790/health` | refused after 1.219s |
| `http://127.0.0.1:8790/health` | 200 in 0.015s |

So every URL here is pinned to `127.0.0.1`. The same fix is applied in the web
app's own API-base resolver.
