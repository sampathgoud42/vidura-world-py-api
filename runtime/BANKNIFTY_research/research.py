...
Pipeline: load 60d of 5m BANKNIFTY -> engineer features -> enumerate 2- and 3-block
...
def main() -> None:
    ...
    log("loading 5m BANKNIFTY bars (yfinance, 60d, incl. pre-market)…")
    bars = data.load_bars()
    ...
    stamp = f"{datetime.now(ZoneInfo(C.TZ)):%Y-%m-%d %H:%M IST}"
    L = [f"# BANKNIFTY 5m Super-Signal research — {stamp}\n",
         f"_{n_days} sessions of 5-minute BANKNIFTY (yfinance, incl. pre-market) · entries {list(C.WINDOWS)} (IST) · "
