#!/usr/bin/env python
"""
nifty_intraday_bot.py — live NIFTY Super-Signal watcher (multi-engine).

Watches completed candles of the NIFTY ETF proxy during the research entry
...
nifty_intraday_signals.csv (IST timestamps despite the template's _cst column
...
Usage:
    python nifty_intraday_bot.py                 # live loop, poll 60s
    python nifty_intraday_bot.py --poll 120
    python nifty_intraday_bot.py --once          # one scan of the latest bar
    python nifty_intraday_bot.py --backfill-today  # emit everything today fired
"""
...
A_BOOK = [
    {"book": "A", "direction": "LONG", "combo": ["above_vwap", "ylow_reclaim"],
     ...
     "acc_strict": 78.6, "acc_tpsl": 100.0, "bt_trades": 14},
    {"book": "A", "direction": "SHORT", "combo": ["below_vwap_v", "mom_ignite_dn"],
     "window": "mid_1030_1300", "nk": 0.5, "vm": 0.8,
     "acc_strict": 66.7, "acc_tpsl": 100.0, "bt_trades": 12},
    {"book": "A", "direction": "LONG", "combo": ["s1_bounce", "cvd_up"],
     "window": "full_0930_1400", "nk": 0.8, "vm": 0.8,
     "acc_strict": 54.5, "acc_tpsl": 100.0, "bt_trades": 11},
    {"book": "A", "direction": "SHORT", "combo": ["yhigh_reject", "hist_turn_dn"],
     "window": "full_0930_1400", "nk": 0.8, "vm": 0.8,
     "acc_strict": 60.0, "acc_tpsl": 100.0, "bt_trades": 10},
]

if __name__ == "__main__":
    engine_common.run_worker(HERE, "nifty", C, data, features, signals, A_BOOK)
