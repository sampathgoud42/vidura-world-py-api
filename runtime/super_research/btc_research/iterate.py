...
def main():
    ...
    def book_stats(sel_idx):
        ...
        st = {"trades": len(b), "wins": wins, "stops": stops,
              ...
              "net_points": round(float(b["pnl"].sum()), 2),
              "max_drawdown_pts": round(float((eq.cummax() - eq).max()), 2),
    ...
    lines = [f"\n\n## ITERATION 2 — greedy precision ensemble ({datetime.now(ZoneInfo(C.TZ)):%Y-%m-%d %H:%M CST})\n",
             ...
             f"- **profit factor:** {st['profit_factor']}  ·  **net:** {st['net_points']} pts  ·  **maxDD:** {st['max_drawdown_pts']} pts",
