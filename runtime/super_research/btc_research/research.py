...
Pipeline: load 60d of 5m BTC -> engineer features -> enumerate 2- and 3-block
composites per direction x noise filters x entry windows -> precomputed
outcome join -> rank. "Success" is strict: TP (+/-1.0 pt) hit within 60 min,
...
def main() -> None:
    ...
    log("loading 5m BTC bars (yfinance, 60d, 24×7)…")
    bars = data.load_bars()
    ...
    L = [f"# BTC 5m Super-Signal research (24×7) — {stamp}\n",
         f"_{n_days} sessions of 5-minute BTC (yfinance, 24×7) · entries {list(C.WINDOWS)} (CST) · "
         f"entry = next bar open · TP {C.TP_POINTS} pt ≤ {C.TP_BARS} bars · SL {C.SL_POINTS} pt ≤ {C.SL_BARS} bars · "
