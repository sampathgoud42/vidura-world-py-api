# iwm_research — IWM 5m 0.30%/4h Super-Signals (PERCENT, custom price+volume)

Deep hunt on IWM: reach **+/-0.30%** before a 0.30% adverse move, **within
...
there). Entries 08:45-14:00 CST, next-bar-open entry, symmetric stop.
    ...
    python iwm_intraday_bot.py --once / --backfill-today / (live loop)
...
**A-book** = compact precision union (9 families, 6 LONG + 3 SHORT): 42 trades,
0 stops, **100.0% TP-before-SL @ 0.7/day** over 60
sessions (results/abook_ensemble.csv). B-book = trimmed results/ensemble.csv.
Book rule at bot load: TP-before-SL > 95% -> A, else B.

Live: super_signal_bot.py --category etf|stock spawns iwm_intraday_bot.py
--once each cycle -> iwm_intraday_signals.csv -> central ledgers -> desk.
