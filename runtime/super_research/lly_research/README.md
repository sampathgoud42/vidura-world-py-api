# lly_research — LLY 5m 0.30%/4h Super-Signals (PERCENT, custom price+volume)

Deep hunt on LLY: reach **+/-0.30%** before a 0.30% adverse move, **within
...
there). Entries 08:45-14:00 CST, next-bar-open entry, symmetric stop.
    ...
    python lly_intraday_bot.py --once / --backfill-today / (live loop)
...
**A-book** = compact precision union (8 families, 3 LONG + 5 SHORT): 37 trades,
0 stops, **100.0% TP-before-SL @ 0.62/day** over 60
sessions (results/abook_ensemble.csv). B-book = trimmed results/ensemble.csv.
Book rule at bot load: TP-before-SL > 95% -> A, else B.

Live: super_signal_bot.py --category etf|stock spawns lly_intraday_bot.py
--once each cycle -> lly_intraday_signals.csv -> central ledgers -> desk.
