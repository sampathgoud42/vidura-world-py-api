# qqq_research — QQQ 5m 0.30%/4h Super-Signals (PERCENT, custom price+volume)

Deep hunt on QQQ: reach **+/-0.30%** before a 0.30% adverse move, **within
...
there). Entries 08:45-14:00 CST, next-bar-open entry, symmetric stop.
    ...
    python qqq_intraday_bot.py --once / --backfill-today / (live loop)
...
**A-book** = compact precision union (9 families, 9 LONG + 0 SHORT): 51 trades,
0 stops, **100.0% TP-before-SL @ 0.85/day** over 60
sessions (results/abook_ensemble.csv). B-book = trimmed results/ensemble.csv.
Book rule at bot load: TP-before-SL > 95% -> A, else B.

Live: super_signal_bot.py --category etf|stock spawns qqq_intraday_bot.py
--once each cycle -> qqq_intraday_signals.csv -> central ledgers -> desk.
