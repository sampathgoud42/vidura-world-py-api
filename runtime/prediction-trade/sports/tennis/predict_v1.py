#!/usr/bin/env python3
"""
predict_v1.py — Tennis Match Prediction Model v2.0 → Kalshi buy signal.
========================================================================
FROZEN STABLE SNAPSHOT (07/06) — reconstructed from predict.py as it stood at
the end of Thursday 07/02 and carried unchanged through Friday 07/03 and
Saturday 07/04 (the profitable weekend window) — BEFORE Sunday 07/05's Gate 1 /
Gate 2 / reversal-flip work and today's (07/06) rollbacks/fixes. Reference only
— not wired into the live bot (see predict.py / predict_v2.py for that).

Reconstructed from code-comment dates (no git commit exists for this window —
every change this week was a direct working-tree edit). Excluded relative to
the current predict.py, all dated 07/05 or 07/06:
  - Gate 1 (07/05): situation A's parity branch here still backs the UNDERDOG
    when the favorite is broken (the pre-Gate-1 behavior).
  - Gate 2 (07/05): _favorite_rebound's set-2 case here is the NARROW original —
    only applies when the favorite LOST set 1, not "any time in set 2".
  - The break-point-against-favorite bug fix (07/05, found while building
    Gate 1/2): this file keeps the ORIGINAL (buggy) condition, since that's
    what actually ran over the weekend.
  - The Bfav BUY-disable (07/06): the rebound rule's BUY branch is ACTIVE here
    (it was live all weekend; the -EV finding and disable came from Monday's
    settlement data, not this window).
  - The odds-vs-rank favorite override (07/06, Grabher/Masarova): not present.
  - The get_player_odds candle-fallback bug fix (07/06): not applicable here
    (that lives in bot_kalshi_sports_v1.py, not this file).

No predict.py edits are dated 07/03 or 07/04 (nothing changed those two days),
so this file is best read as "state after Thursday's session," carried forward.
